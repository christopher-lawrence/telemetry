...
describe('jQuery 1.4 parser', () => {
    ...
    it('parses the test file and returns the correct number of events found (5)', (done) => {
        setTimeout(() => {
            ...
            expect(result.length).to.be.equal(5, 'parser did not return correct number of elementListeners');
            done();
        }, 500);
    });
    ...
    it(`reports the correct version (${$.fn.jquery})`, (done) => {
        setTimeout(() => {
            const parser = new JQueryParser();
            const result = parser.parse(allElements);
            expect(parser.name()).to.be.equal('jQuery 1.4.4', 'Incorrect version reported');
            done();
        }, 500);
    });
});
